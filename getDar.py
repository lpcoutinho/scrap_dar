...
class GetDar:
    ...
    def init_driver(self):
        ...
        logger.info(
            f"Tempo para instalar o plugin: {self.tempo_download_plugin} segundos"
        )
        ...
        logger.info(
            f"Tempo para configurar o Chrome: {self.tempo_chrome_config} segundos"
        )
    ...
    def show_data(self, inscricao, exercicio):
        ...
        logger.info(
            f"Tempo para baixar o plugin: {self.tempo_download_plugin} segundos"
        )
        logger.info(
            f"Tempo para configurar o Chrome: {self.tempo_chrome_config} segundos"
        )
        logger.info(
            f"Tempo para resolver o Captcha: {self.tempo_resolucao_captcha} segundos"
        )
    ...
    def atualizar_dados(self, novos_dados):
        ...
        novos_dados = pd.concat(
            [self.dados_df, pd.DataFrame([novos_dados])], ignore_index=True
        ).drop_duplicates()

        # Salve o DataFrame no arquivo CSV
        novos_dados.to_csv(
            "data/dados.csv", index=False, sep=";", mode="a", header=False
        )
    ...
    def atualizar_performance(self, nova_performance):
        ...
        # Adicione os novos dados ao DataFrame
        self.dados_performance = pd.concat(
            [self.dados_performance, pd.DataFrame([nova_performance])],
            ignore_index=True,
        ).drop_duplicates()

        # Salve o DataFrame no arquivo CSV
        self.dados_performance.to_csv(
            "data/performance.csv", index=False, sep=";", mode="a", header=False
        )
    ...
    def fill_and_scrape(self, inscricao):
        ...
        try:
            ...
            time.sleep(
                2
            )  # Aguarde para que os resultados sejam carregados (ajuste conforme necessário)
        ...
        try:
            ...
            self.tempo_consulta = (
                tempo_apos_esperar_consultar - tempo_apos_clicar_consultar
            )
            ...
            try:
                ...
                try:
                    ano_element = self.driver.find_element(
                        By.XPATH, '//*[@id="mat-select-value-1"]/span/span'
                    )
                ...
                performance = {
                    "* Download_plugin": self.tempo_download_plugin,
                    "Resolve_Captcha": self.tempo_resolucao_captcha,
                    "Consulta_DAR": self.tempo_consulta,
                    "Get_DAR": None,
                ...
                inscricao_output = self.driver.find_element(
                    By.XPATH, inscricao
                ).text.strip()
                nome_output = self.driver.find_element(By.XPATH, nome).text.strip()
                endereco_output = self.driver.find_element(
                    By.XPATH, endereco
                ).text.strip()
                ...
                for elemento in elementos:
                    ...
                    valor_total_element = elemento.find_element(
                        By.XPATH, xpath_valor_total
                    )
                    ...
                    try:
                        ...
                        cod_bar_element = self.driver.find_element(
                            By.CLASS_NAME, class_cod_bar
                        )
                    ...
                    nome_arquivo_pdf = (
                        f"{pdf_dir}/{inscricao_output}_{ano_output}_{cota_output}.pdf"
                    )

                    # Renomear o arquivo PDF após o download
                    novo_nome = os.rename(
                        f"{pdf_dir}/RelatorioDAR.pdf", nome_arquivo_pdf
                    )
                    ...
                    novos_dados = {
                        ...
                        "DAR": True,
                        "Nome": nome_output,
                        "Endereço": endereco_output,
                        "Cota": cota_output,
                        "Valor": valor_output,
                        "Multa": multa_output,
                        "Juros": juros_output,
                        "Outros": outros_output,
                        "ValorTotal": valor_total,
                        "Cod_Barras": cod_bar_output,
            ...
            logger.info(
                f"Ocorreu um erro ao processar a inscrição {inscricao_output}: {str(e)}"
            )
